...
function searchTransactions() {
  ...
  document.getElementById('modalTitle').innerHTML = "Search Transactions In " + currentYear

  document.getElementById('modalList').innerHTML = "<div class=\"cntr\"><div class=\"cntr-innr\"><label class=\"search\" for=\"inpt_search\"><input id=\"inpt_search\" type=\"text\"/></label></div></div><hr>"

  list.style.listStyleType = 'none'
  modal.appendChild(list)

  let transactions = allTransactionsForCurrentYear()
  ...
  transactions.forEach(item => {
    ...
    li.innerHTML = item
    list.appendChild(li);
    totalSpent += extractCharge(item)
  })
  modalFooter.innerHTML = "<p id=\"totalSpentModal\" class=\"mr-auto\">Total Spent: " + totalSpent.toFixed(2) + "<p><button type=\"button\" class=\"btn btn-secondary btn-danger\" data-dismiss=\"modal\">Close</button>"
  ...
  jQuery('#inpt_search').keyup(function () {
    let targetWord = transactions.filter(transaction => transaction.includes(jQuery('#inpt_search').val().toLowerCase()));
...
function allTransactionsForCurrentYear() {
  let fullTransactionList = []
  for (year of statementsByYear) {
    if (year[0] == currentYear) {
      year[1].forEach(month => {
        month.pie.categories.forEach(category => {
          category[2].forEach(trxn => fullTransactionList.push(trxn.toLowerCase()))
        })
      })
    }
  }
...
function extractCharge(transaction){
  let endOfCharge = transaction.length
  let startOfCharge = transaction.lastIndexOf("-")
  let charge = transaction.substring(startOfCharge, endOfCharge) 
  return parseFloat(charge.replace(/,/g, ""))
